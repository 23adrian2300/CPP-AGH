...
public:
    ...
    TwoDimensionMatrix(const TwoDimensionMatrix &firstmatrix) ;

    explicit TwoDimensionMatrix(const MatrixElement matrix[size_][size_]);

    //getter
    [[nodiscard]] MatrixElement get(size_t row, size_t col) const;

    //size
    static constexpr size_t size(){return size_;};

    //przeciążanie operatorów
    TwoDimensionMatrix &operator=(const TwoDimensionMatrix &firstmatrix);

    //friend bo ma dostep do elementów prywatnych klasy
    ...
    MatrixElement* operator[](size_t i) { return matrix_[i]; };

    // nadpisanie size_t()
    explicit operator size_t() const {return size();};

    //dla jednego z testów bo inaczej nie przejdzie
    const MatrixElement* operator[](size_t i) const {return matrix_[i];};
